...
#define FIRST_BULK_THICKNESS 2.0   //[nm] thickness of first bulk index (sized so that its centroid corresponds to mean projective range of incident H ions)
#define ELEMENT_THICKNESS 2.0 // [nm] thickness of bulk spatial element
...
#define DIVIDING_AREA 0.04e-12  // [cm^2] Area of the surface in between volume elements.
...
#define POINTS 1120 // number of elements: one surface(Point 0), one subsurface (Point 1), one implantation bulk element (Point 2), other bulk elements, other elongated bulk elements
...
#define FIRST_EXP_INDEX 1002 // the first mesh element where the length starts to increase exponentially (to reach experiment lengths)
...
const double AVG_ION_EN[POINTS] = {0};
